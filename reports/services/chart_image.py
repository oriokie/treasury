...
def _line_chart(title, labels, series, *, width=760, height=300):
    ...
    img = Image.new("RGB", (width, height), PAPER)
    d = ImageDraw.Draw(img)
    d.text((20, 14), title, font=f_title, fill=FOREST)
    ...
    def xy(i, v):
        ...
        x = pad_l + plot_w * (i / n)
        y = pad_t + plot_h * (1 - (float(v or 0) - lo) / (hi - lo))
        return x, y

    for g in range(5):                       # horizontal grid + axis values
        gy = pad_t + plot_h * g / 4
        d.line([(pad_l, gy), (width - pad_r, gy)], fill=(232, 236, 233))
        gv = hi - (hi - lo) * g / 4
        d.text((8, gy - 7), _money(gv), font=f_axis, fill=MUTED)
    step = max(len(labels) // 8, 1)          # sparse x labels
    for i, lab in enumerate(labels):
        if i % step == 0:
            x, _ = xy(i, lo)
            d.text((x - 12, height - pad_b + 6), str(lab)[:8], font=f_axis,
                   fill=MUTED)
    ...
    for lbl, _vals, color in series:
        d.rectangle([lx, ly, lx + 14, ly + 12], fill=color)
        d.text((lx + 20, ly - 2), str(lbl), font=f_axis, fill=INK)
        lx += 24 + int(d.textlength(str(lbl), font=f_axis)) + 16
    return img
