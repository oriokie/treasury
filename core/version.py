...
WHATS_NEW = {
    "3.47.0": "A campaign can now show how it is doing on its own public pledge link. Tick "
              ...
              "bar against the goal — the standing that moves the next giver. It is off until you "
              "turn it on, because whether an appeal's running total belongs outside the church is "
              "the church's decision and not a default. Two things it will not do: it counts only "
              "pledges that have been approved, so a promise someone typed into the form themselves "
              "cannot raise the figure the next giver is looking at before anyone has checked it; "
              "and it stays silent until there is something to report, since an appeal that opens "
              "with \"0 pledges\" discourages the first giver rather than moving them. The campaign "
              "page tells you exactly what the link publishes, so it is never a surprise to whoever "
              "shares it.",
    ...
    "3.43.0": "Leaders can now import a batch of pledges for their own fund, with the member field a "
              ...
              "that the campaign is fixed to one the leader's fund owns and re-checked when the rows "
              "are applied — so neither a Campaign column in a spreadsheet nor a field in a form can "
              "move a pledge onto another fund. Imported pledges are drafts, as the treasurer's are: "
              "a leader gathers promises, and making them count is still an approval. The fund's "
              "pledge page no longer calls itself read-only, marks drafts as awaiting the treasurer, "
              "and says what to do next when it is empty. Pledge messages are now the church's own "
              "words: both the reminder and a new thank-you for pledging are editable under "
              "Settings, and both can be read in full before anything is sent — on the batch page as "
              "a live preview with the character and SMS segment count, and on a single send as the "
              "actual message rather than 'are you sure?'. Thanking someone for a promise is a "
              "different act from chasing them for it, and a church whose members only hear from the "
              "treasurer when they are behind has taught them what a message from the treasurer "
              "means. A member who has opted out receives neither. Auto-matching now starts at the "
              "pledge date: there was a seven-day window BEFORE it, so anyone who gave on the "
              "Sabbath and pledged the following week had that earlier gift counted toward the "
              "pledge, crediting them twice and flattering the campaign. And where a report shows "
              "total receipts beside a per-fund statement, the difference between them — money "
              "banked but not yet receipted to a fund — is now a line of its own rather than an "
              "unexplained gap, with the reconciliation page setting out why bank money and "
              "envelopes cannot always be matched one to one.",
    ...
    "3.41.2": "Fixes a bank reconciliation that lost money receipted on Sabbath. Marking a bank line "
              ...
              "Sabbath — so tithe banked on Friday the 31st and receipted on Saturday the 1st was, "
              "on a 31 July worksheet, in no fund and also excluded from receipts still pending. It "
              "was counted nowhere, and the worksheet was short by exactly that amount. Every figure "
              "on a worksheet is now read from its own statement date rather than from today: on "
              "Friday night the money was at the bank and in no fund, whichever route later "
              "receipted it, so a Friday reconciliation says so and goes on saying so however long "
              "afterwards it is prepared. Saturday's own worksheet needs no such line, because by "
              "then the fund has the money. This also makes a worksheet for a past date stable — "
              "nothing done afterwards moves figures that were settled at the time, where before the "
              "suspense line shifted every time something was allocated. Worksheets already prepared "
    ...
    "3.41.0": "A loan converted to a donation is no longer reported as money spent. The lender lets "
              ...
              "worked out \"expenditure\" for themselves as \"everything except a remittance\", which "
              "let the contra through — so those pages showed spending the church never did and "
              "disagreed with the Income & Expenditure statement, which had always excluded the "
              "whole liability class. All of them now read the one definition. The board pack's "
              "headline figures are the four a board actually wants: total receipts, total trust "
              "funds, total expenditure and trust still to remit — drawn from the same figures as "
              "the collections table beneath them, so the headline and the first table cannot "
              "disagree. It also carries the full Statement of Financial Position rather than a "
              "summary, with current assets separated from fixed, the trust liability split by "
              "whether it has been receipted, borrowings split current against long-term, and a "
              "funds-employed section showing what the net assets consist of. Reports can now be "
              "read as at a date on what was known THEN rather than on what is known now: a credit "
              "banked in July and receipted in August was, on 31 July, sitting unreceipted, and the "
              "new \"position as it stood on the date\" option says so — suspense carries it, the "
              "fund does not, and the bank reconciliation has something to explain the gap with. "
              "Off by default, and the page says which basis it was drawn on. Bank reconciliation: "
              "a new worksheet now suggests the closing balance from the bank register's own figure "
              "for the date being reconciled and re-suggests it when the date changes, leaving the "
              "field blank when the register cannot answer rather than inventing one; receipts "
              "still awaiting allocation at the statement date now appear as a reconciling item, "
              "which is what a month with anything in the review queue needed to balance; and the "
              "board pack shows the worksheet the treasurer actually prepared instead of computing "
              "a second reconciliation of its own. Entering a batch of expenses twice — the second "
              "click on a slow save — now records one batch: the form carries a token claimed once, "
              "and the button says what it is doing. Turning every development pattern off now "
              "turns development detection off; the fallback meant for a database with no patterns "
              "could not tell that from a deliberately emptied one, so switching detection off "
              "quietly switched the built-in version of it back on. Campaigns can thank the groups "
              "that reached their target, not only chase the ones that have not. Bank debits "
              "awaiting classification now show their count in the sidebar and on the page, the way "
              "the credit review queue does. And the Word export is set like the report on screen "
              "rather than in a style of its own.",
    "3.40.0": "The Board / Treasurer's Report has been rebuilt as a short pack meant to be printed "
              "and handed round a board table. It carries only what a board is accountable for: key "
              "figures and an executive summary; a collections summary and a trust fund summary; the "
              "statements of fund balances, financial position and cash flows; then the trial "
              "balance, the bank reconciliation, matters for the board, and signatures. Charts, "
              "budget variance and department analysis stay on the fuller Treasurer's pack, where "
              "there is room for them. The two summary tables show a single figure column when the "
              "period sits inside one month and month by month when it does not, so \"for July\" and "
              "\"for the year\" are the same report, and the collections figures agree with the "
              "standalone Collections Summary report for the same dates. The bank reconciliation is "
              "now drawn at the period end against the bank's own balance for that date; where no "
              "statement covering that date has been imported it says the account is not reconciled "
              "and prints no figure, rather than quietly measuring the cash book against some other "
              "day's balance and calling the difference a reconciliation. The statements read "
              "shorter too. Fund balances now puts the largest fund first, leaves out dormant funds "
              "(nothing held and nothing moved), drops the Transfers column entirely when no money "
              "moved between funds, and shows a blank instead of 0.00 where a fund neither received "
              "nor paid — everywhere that statement appears, not only in this pack — and the trial "
              "balance leaves an account's unused column blank. Inside the board pack only, lines "
              "with no balance are left off the position and cash flow statements; the standalone "
              "statements, where \"nil\" is itself worth stating, are unchanged. A loan converted to "
              "a donation or written off now appears as a non-cash memo below the cash flow "
              "statement: no money moved, so it is kept out of operating receipts and is no longer "
              "netted against loan receipts, which remain the cash actually borrowed. Every section "
              "carries an explanation written from its own figures which a treasurer can rewrite in "
              "place, or have the AI assistant draft where the assistant is switched on; wording is "
              "held per section and per period, so editing July leaves June alone, and emptying the "
              "box brings the generated words back. It travels with the PDF and Word exports, and is "
              "now available on every engine report rather than this one alone. Printing was "
              "designed for rather than tolerated: A4 portrait, a heading and page numbers on every "
              "sheet, table headings repeated across page breaks, no row or section split down the "
              "middle, and rules instead of grey fills so the pack survives a photocopier.",
