...
*Updated Phase 7 (complete financial-statement migration, consistency audit,
snapshot foundation). Tracks migration of reporting code onto the Financial
Metrics Registry (`core/metrics.py`) via the Semantic Reporting Layer and the
component-based Generic Report Engine (`core/reporting`).*

---

## Phase 7 update

**Registry grew to 26 metrics** — added `financing_activity`,
`loan_retirement_income`, `remittances_total` so the Cash Flow Statement reads
every figure from the registry. Canonical implementations in
`loans.services.reporting` and `reports.services.balances`.

**Financial statements migrated (parallel-run; legacy untouched):** Cash Flow
(`cash_flow_v2`, reconciles opening + net change == closing), Statement of Fund
Balances (`fund_balances_v2`, total == closing cash), Budget vs Actual
(`budget_vs_actual_v2`, totals equal the canonical budget service). These join
the Phase 6 migrations (Income & Expenditure, Trial Balance, Financial Position
summary, Board Report). Ten engine reports now registered.

**Reporting Consistency Audit** (`consistency_audit`) — a cross-report
reconciliation proving trial balance balances, accounting equation holds, cash
flow reconciles, fund balances tie to closing cash, and dashboard figures equal
report metrics. All checks pass on seeded data.

**Snapshot foundation** — an immutable `ReportSnapshot` model + service captures
any engine report with a deterministic payload checksum, provenance and versions;
`verify_snapshot` detects drift. No scheduling; no change to report behaviour.

**Dashboard reconciliation confirmed** — the executive dashboard already shares
`core.metrics.income_credits` (the definition `total_income` wraps), so its
figures reconcile with the reports by construction (verified by test). The main
`DashboardView` reads through `ReportContext` (Phase 6).
